...
export async function retrieve(input: {
  ...
  const diagnostics = [
    ...
    ...Object.values(input.index.files)
      .filter((file) => file.diagnostics.length && matchesPaths(file.path, input.input.paths))
      .flatMap((file) => file.diagnostics.map((diagnostic) => `${file.path}: ${diagnostic}`)),
  ]
  const chunks = Object.entries(input.index.chunks)
    .flatMap(([key, chunk]) => key === chunk.id ? [chunk] : (diagnostics.push(`chunk key mismatch: ${key} contains ${chunk.id}; chunk skipped`), []))
  ...
  const candidateCount = Math.max(topK * 3, topK)
  ...
  const hyde = input.options.hyde.enabled && (bestScore ?? -1) < input.options.hyde.threshold
    ? await input.generateHyde(input.input.query)
      .then((text) => input.embed(text))
      .then((vector) => ({ scored: searchVectors(vector, vectors, candidateCount), hydeUsed: true, diagnostics: [] as string[] }))
      .catch((error) => ({ scored: initial, hydeUsed: false, diagnostics: [`HyDE failed: ${error instanceof Error ? error.message : String(error)}`] }))
    : { scored: initial, hydeUsed: false, diagnostics: [] }
  const seenParentRanges = new Set<string>()
  const results = (await Promise.all(hyde.scored.slice(0, topK).flatMap(async (result) => {
    const chunk = chunksById[result.id]
    if (!chunk) return []

    const source = await input.readSource(chunk.filePath)
      .then((text) => ({ text, ok: true }))
      .catch(() => (diagnostics.push(`source read failed for ${chunk.filePath}; parent context omitted`), { text: chunk.text, ok: false }))
    if (source.ok && !indexedChunkMatchesSource(source.text, chunk)) diagnostics.push(`source mismatch for ${chunk.filePath}:${chunk.id}; parent context omitted`)
    const context = input.input.includeParents === false
      ? { breadcrumbs: [] }
      : source.ok && indexedChunkMatchesSource(source.text, chunk)
        ? expandWithParentContext({ chunk, symbols: input.index.symbols, source: source.text, maxContextChars })
        : { breadcrumbs: breadcrumbsFor(chunk, input.index.symbols) }
    ...
    if (!result.parentRange) return result
    const parentRangeKey = `${result.filePath}:${result.parentRange.byteStart}:${result.parentRange.byteEnd}:${result.parentText}`
    if (seenParentRanges.has(parentRangeKey)) return { ...result, parentText: undefined, parentRange: undefined }
...
function matchesPaths(filePath: string, paths: string[] | undefined) {
  ...
  return paths.some((filter) => filePath === filter || filePath.startsWith(filter.endsWith("/") ? filter : `${filter}/`))
}
