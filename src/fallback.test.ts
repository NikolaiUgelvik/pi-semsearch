...
describe("fallbackChunks", () => {
  test("keeps small files as one chunk", () => {
    const [chunk] = fallbackChunks({ filePath: "src/a.ts", language: "typescript", text: "const a = 1\n", maxNonWhitespaceChars: 20 })
  ...
  test("splits large files on line boundaries", () => {
    const chunks = fallbackChunks({ filePath: "src/a.ts", language: "typescript", text: "aaaa\nbbbb\ncccc\n", maxNonWhitespaceChars: 8 })
  ...
  test("uses UTF-8 byte ranges for multibyte text", () => {
    ...
    expect(chunk.range).toEqual({ byteStart: 0, byteEnd: new TextEncoder().encode(text).length, lineStart: 1, lineEnd: 2 })
  ...
  test("splits long single lines within the non-whitespace budget", () => {
    ...
    expect(chunks.map((chunk) => chunk.id)).toEqual(chunks.map((chunk) => `src/a.ts:${chunk.range.byteStart}:${chunk.range.byteEnd}`))
    expect(chunks.map((chunk, index) => chunk.previousSiblingChunkId)).toEqual(chunks.map((_, index) => chunks[index - 1]?.id))
    expect(chunks.map((chunk, index) => chunk.nextSiblingChunkId)).toEqual(chunks.map((_, index) => chunks[index + 1]?.id))
  })
