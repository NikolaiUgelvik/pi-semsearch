...
export function createCastPluginForTest(dependencies = {}) {
    return async (input, rawOptions) => {
        ...
        const wrapIndexingStore = (indexStore) => {
            if (!indexStore) {
            ...
            if (hasVectorCandidateStore(indexStore)) {
                wrapped.searchVectorCandidates = (queryEmbedding, topK, paths) => wrapStoreOperation(() => indexStore.searchVectorCandidates(queryEmbedding, topK, paths));
            }
            return wrapped;
        };
        ...
        const semanticSearchOutput = async (args, context) => {
            const embedding = options.embedding;
            if (!embedding) {
        ...
        const generateOpenCodeHyde = async (query, context) => {
        ...
        return {
            config: async (cfg) => {
                const toolOutput = cfg.tool_output;
                outputLimits = {
...
async function createHydeSession(client, context) {
...
async function hydePromptText(client, hydeSessionID, input) {
    const parameters = {
...
function nonEmptyHydeText(parts) {
    const text = parts
...
function deleteHydeSession(client, hydeSessionID, context) {
