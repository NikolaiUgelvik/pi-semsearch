import { textForByteSlice } from "../parsing/range.js";
...
function chunkMatchesSource(source, chunk) {
    return textForByteSlice(source, chunk.range.byteStart, chunk.range.byteEnd) === chunk.text;
}
function chunkBreadcrumbs(chunk, symbols) {
    return chunk.symbolIds.flatMap((id) => (symbols[id] ? [`${symbols[id].kind} ${symbols[id].name}`] : []));
}
function optionalChunk(chunks, id, symbols, key) {
    const chunk = id ? chunks[id] : undefined;
    return chunk ? { [key]: summarizeChunk(chunk, symbols) } : {};
}
function labelForChunk(chunk, symbols) {
    if (chunk.kind === "file") {
...
function symbolBreadcrumbs(chunk, symbols) {
